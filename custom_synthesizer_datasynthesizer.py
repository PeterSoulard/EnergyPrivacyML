...
CUSTOM_ARGUMENTS = [{"epsilon": epsilon, "k": k} for epsilon in np.linspace(0.1, 2.0, 10) for k in range(1, 3) for _ in range(30)]
...
def generate_data(training_data: pd.DataFrame, arguments: dict) -> pd.DataFrame:
    ...
    for f in ["temp.csv", "description_temp.json", "description.json", "synthetic_data_temp.csv"]:
        if os.path.exists(f):
            os.remove(f)
